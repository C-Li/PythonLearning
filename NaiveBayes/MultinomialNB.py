...
class MultinomialNB(NaiveBayes):
    ...
    def feed_data(self, x, y, sample_weight=None):
        ...
        feat_dics = [{_l: i
                      for i, _l in enumerate(feats)} for feats in features]
        label_dic = {_l: i for i, _l in enumerate(set(y))}
        # 利用转换字典更新训练集
        x = np.array([[feat_dics[i][_l] for i, _l in enumerate(sample)]
                      for sample in x])
        y = np.array([label_dic[yy] for yy in y])
        # 利用Numpy的bincount方法，获得各类别的数据的个数
        cat_counter = np.bincount(y)
        # 记录各维度特征的取值个数
        n_possibilities = [len(feats) for feats in features]
        # 获得各类别数据的下标
        labels = [y == value for value in range(len(cat_counter))]
        # 利用下标获取记录按类别分开后的输入数据的值
        labelled_x = [x[ci].T for ci in labels]
        # 更新模型的各个属性
        self._x, self._y = x, y
        self._labelled_x, self._label_zip = labelled_x, list(
            zip(labels, labelled_x))
        (self._cat_counter, self._feat_dics,
         self._n_possibilities) = (cat_counter, feat_dics, n_possibilities)
        ...
        for dim, _p in enumerate(self._n_possibilities):
            if sample_weight is None:
                self._con_counter.append([
                    np.bincount(xx[dim], minlength=_p)
                ...
                self._con_counter.append([
                    np.bincount(
                        xx[dim],
                        weights=sample_weight[label] /
                        sample_weight[label].mean(),
                        minlength=_p) for label, xx in self._label_zip
                ])
    ...
    def _fit(self, lb):
        ...
        for dim, n_possibilities in enumerate(self._n_possibilities):
            data[dim] = [[(self._con_counter[dim][c][p] + lb) /
                          (self._cat_counter[c] + lb * n_possibilities)
                          for p in range(n_category)]
        ...
        def func(input_x, tar_category):
            rs = 1
            # 遍历各个维度，利用data和条件独立性假设计计算联合条件概率
