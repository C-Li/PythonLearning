...
class NaiveBayes:
    """
        ...
        self._labelled_x:记录按类别分开以后的输入数据的数组
        self._label_zip:记录类别相关信息的数组，视具体算法，定义会有所不同
        self._cat_counter:核心数组，记录第i类数据的个数（cat:category）
        self._con_counter:核心数组，用于记录数据条件概率的原始极大似然估计（con:conditional）
    ...
    def get_prior_probability(self, lb=1):
        return [(_c_num + lb) / (len(self._y) + lb * len(self._cat_counter))
                for _c_num in enumerate(self._cat_counter)]
    ...
    # get_raw_result=False则输出类别，否则输出后验概率
    def predict_one(self, x, get_raw_result=False):
        # 在进行预测前，要把新的输入数据数值化
        # 如果输入的是Numpy模型，要先将它转换成Python的数组
    ...
    # 定义预测多样本的函数，本质是不断调用上面的predict_one函数
