...
class Cluster:
    """
        ...
        self._counters:类别变量的计数器，记录第i类数据的个数
    ...
    def __init__(self, x, y, sample_weight=None, base=2):
        ...
        if sample_weight is None:
            ...
            self._counters = np.bincount(
                self._y, weights=sample_weight * len(sample_weight))
    ...
    def ent(self, ent=None, eps=1e-12):
        ...
        _ent_cache = max(
            eps, -sum([
                _c / _len * math.log(_c / _len, self._base) if _c != 0 else 0
                for _c in ent
            ]))
    ...
    def con_chaos(self, idx, criterion="ent", features=None):
        ...
        # 由于调用set方法比较耗时，在决策树实现时应该努力将features传入
        if features is None:
            features = set(data)
        # 获得该维度特征取各值时对应的数据下标
            ...
            if self._sample_weight is None:
                ...
                _chaos = _method(
                    Cluster(
                        tmp_data,
                        tar_label,
                        _new_weights / np.sum(_new_weights),
                        base=self._base))
